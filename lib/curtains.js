...
function cellularCurtains() {
  let ruleNumber = helpers.choice([60, 102, 99, 26, 129])
  let rule = cellular.ruleForInt(ruleNumber)
  let length = 40 + 2 + 20
  let state = helpers.randomBinary(length)
  return (grimer) => {
    state = cellular.iterate(state, rule)
    let dreamified = cellular.translate(state, [9, 1])
    return {
      left: grimes.grimeString(dreamified.slice(0, 8), grimer),
      right: grimes.grimeString(dreamified.slice(-8), grimer)
    }
  }
}
