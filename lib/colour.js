...
function getColourScheme () {
  let h = Math.random() * 360 // ((Math.random() * 240) + 340) % 360
  let s = floatBetween(0.2, 0.4)
  let v = floatBetween(0.5, 0.6)
  ...
  let hintS = s + 0.23
  if (hintS > 1) hintS = 1
  let hint = toHexNotation(hsvToRgb((h + 350) % 360, hintS, hintV))

  let grime = toHexNotation(hsvToRgb(h, 1, v))

  let wall = toHexNotation(hsvToRgb((h + 40) % 360, floatBetween(s, 0.5), floatBetween(0.05, 0.2)))
